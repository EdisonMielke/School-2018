...
This file provides the Appt and Agenda classes
'''

from datetime import datetime

class Appt:
    """ An appointment with a start time, end time and title.
    The start and end time will be on the same time.
    Usage:
    ...
    def __init__(self, start: datetime, finish: datetime, desc: str):
        """An appointment with a start time and finish time, with description desc.
        Start and finish should be same day
        """
    ...
    def __eq__(self, other: 'Appt') -> bool:
        """Equality only means same time period, with no other description in this case"""
        return self.start == other.start and self.finish == other.finish

    def __lt__(self, other: 'Appt') -> bool:
        """Checks to see if self Appt is before other Appt"""
        return self.start < other.start and self.finish < other.finish
    
    def __gt__(self, other: 'Appt') -> bool:
        """Checks to see if self Appt is after other Appt"""
        return self.start > other.start and self.finish > other.finish

    def overlaps(self, other: 'Appt') -> bool:
        """Is there a non-zero overlap between these periods?"""
        if (self.start < other.start < self.finish) or (other.start < self.start < other.finish):
            return True
    ...
    def intersect(self, other: 'Appt') -> 'Appt':
        ...
        minfinish = min(self.finish, other.finish)
        return Appt(maxstart,minfinish,f"{self.desc} and {other.desc}")
