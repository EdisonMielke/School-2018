...
Credits:  Imported functions from key 4-1

Test Functions to make sure they are correct
'''
...
def test_checkTone(f):
    ...
    for i in range(len(testCases)):
        print("Checking", f.__name__,'', end='')
        if f(testCases[i][0]) == (testCases[i][1]):
            print("Test is a match!")
        else:
            print("Test is not a match...")
    return None
def test_decode(f):
    testCases = (('lo', 43),('hi', 27),('bomelela', 3464140),('bomeluco', 3464408),('', -1),('abcd', -1),('diju', 1234),)
    for i in range(len(testCases)):
        print("Checking", f.__name__,'', end='')
        if f(testCases[i][0]) == (testCases[i][1]):
            print("Test is a match!")
...
def main():
    ...
    print("")
    test_decode(alphapinDecode2)
    print("")
    test_checkTone(checkTone)
    print("")
    test_checkTone(checkTone2)
    return None
main()
