...
class If(Control):
    ...
    def gen(self, context: 'Context', target: str):
        ...
        else_1 = context.new_label("else")
        fi_2 = context.new_label("fi")
        self.cond.condjump(context, target, else_1, jump_cond=False)
        Assign.gen(self.thenpart, context, target)
        context.add_line(f"   JUMP   {fi_2}-")
        context.add_line(f"{else_1}:")
        Assign.gen(self.elsepart,context,target)
        context.add_line(f"{fi_2}:")
